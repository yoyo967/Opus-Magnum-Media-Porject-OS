...
| **L2 — Kontext-Schicht** | kanonische **Mirrou Knowledge Base** (Brand/ICP/Benchmarks/Angebot/Voice/Gewinner-Outputs); **jedes Tool grundet automatisch** darauf | 🟡 **Rollout weit:** `tenants/mirrou/knowledge.ts` + `buildMirrouContext()`/`withMirrouKnowledge()` — **18/29 Tools geerdet.** Welle 1–2: Baumeister, Prometheus, Markenwächter, Späher, Auditor, Resonator-Copy, Visionär-Bildanalyse. **Welle 3 (2026-06-06):** Stratege, Persona, Konversator, Personalisator, Diplomat, Sequenzer, Kolorit, Analytiker, Experimentator, Dirigent, Berichterstatter (alle Strategie-/Content-/Voice-Tools → `systemInstruction: MIRROU_KNOWLEDGE`). **Rest ungeerdet (bewusst):** reine Bild/Video-Gen (Visionär `generateImages`, Animator) + Utility/Prozess (Observatorium, Conductor, Chronist, Publisher, Taktgeber, Auditorium, Ensemble, Gespraechsleiter, Orakel). **Design-Entscheidung offen (decision-gated, SaaS):** KB trennen in *Methode/Compliance (immer an)* vs. *aktive Marke (Mirrou default, pro Kunde austauschbar)* = Multi-Brand. **L2.5 erledigt (2026-06-07):** KB nach Firestore (`tenants/{tenantId}/knowledge/main`), im Cockpit **live editierbar** (Header → System & Knowledge → „Knowledge Base"); Code-Default = Seed/Fallback. `MIRROU_KNOWLEDGE` ist ein Live-Binding (`export let` + `setMirrouKnowledge`) → Edits propagieren via reaktivem `onSnapshot` ohne Redeploy an alle 18 Tools + Orchestrator |
| **L3 — Orchestrierung** | Tools **verketten**; echte **Playbooks** | 🟡 **2 Ketten live.** Playbook 2 (2026-06-05): Baumeister-Brief → Visionär/Markenwächter/Auditor. **Lead-Playbook (2026-06-06): Lead → Qualifizieren → Brief → Konzept → Compliance** — Lead-Inbox: „🎯 Qualifizieren (KI)" (grounded ICP-Verdikt) + „→ Creative Brief" → `setToolInput({tool:'baumeister', fields:{brand}})` → Baumeister prefillt (liest jetzt `toolInput`, `ToolInput.fields` ergänzt) → bestehende `chainTargets` → Visionär/Auditor. Mensch ist Checkpoint an jeder Stufe. **CommandBar-Orchestrator (2026-06-06):** ⌘K-MAGNUM-AI auf Mirrou-KB geerdet + Tool-Zweck-Katalog → routet NL-Befehle zuverlässig (navigateTo/createTask/setToolInput) und befüllt Tools mit mirrou-konformen Prompts statt nur zu navigieren. Nächste (L4): Lead-rein-Trigger + Feedback-Loop |
| **L4 — Proaktiv + Lernend** | Trigger (Lead rein → Auto-Entwurf, Mensch gibt frei) + Feedback-Loop (was gewinnt → zurück in die KB) | 🟡 **Schritt 1 live (2026-06-07): proaktive Auto-Qualifizierung.** Lead-Inbox qualifiziert beim Laden neue Leads automatisch (grounded, gedeckelt 5/Lauf) + **persistiert** nach Firestore (`leads/{id}.qualification`, einmal rechnen, team-geteilt) → Mensch sieht Leads schon qualifiziert (✨-Tag), gibt per „→ Brief" frei. **Bewusst frontend-seitig** (kein Eingriff in den produktiven `POST /api/lead`-Pfad der Website). Offen: echter On-Arrival-Backend-Trigger (braucht google-genai im Backend) + Feedback-Loop (Gewinner → KB) |
