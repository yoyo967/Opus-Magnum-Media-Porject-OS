...
| **L3 — Orchestrierung** | Tools **verketten**; echte **Playbooks** | 🟡 **erste Kette live** (2026-06-05): Playbook 2 — Baumeister-Brief → „→ An nächstes Tool senden" → Visionär (Visual aus hook.visual_idea) / Markenwächter (Brand-Check) / Auditor (Compliance). Mechanik: `OutputActions.chainTargets` → `setToolInput` → Auto-Nav (App.tsx); Ziel-Tools lesen `toolInput` + prefillen. Nächste: Lead-Playbook verketten + CommandBar als Voll-Orchestrator |
