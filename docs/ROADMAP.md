...
| **L2 — Kontext-Schicht** | kanonische **Mirrou Knowledge Base** (Brand/ICP/Benchmarks/Angebot/Voice/Gewinner-Outputs); **jedes Tool grundet automatisch** darauf | 🟡 **Rollout läuft:** `tenants/mirrou/knowledge.ts` + `buildMirrouContext()`/`withMirrouKnowledge()` — **7 Kontexte geerdet** (Baumeister, Prometheus, Markenwächter, Späher, Auditor, Resonator-Copy, Visionär-Bildanalyse). **Bewusst NICHT geerdet:** reine Bild/Video-Generierung (Visionär `generateImages`, Animator `generateVideos`) — würde Kunden-Paletten überschreiben; Brand-Direktion kommt aus dem (kundenspezifischen) Brief. **Design-Entscheidung offen:** KB trennen in *Methode/Compliance (immer an)* vs. *aktive Marke (Mirrou default, pro Kunde austauschbar)* = Multi-Brand für Kundenarbeit. Dann L2.5 (KB nach Firestore, editierbar) |
