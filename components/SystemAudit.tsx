...
const SystemAudit: React.FC<SystemAuditProps> = ({ navigateTo, isEmbedded }) => {
    ...
    useEffect(() => {
        ...
        const runAudit = async () => {
            const bootSequence = [
                ...
                "BigQuery Uplink established (Sandbox Mode)...",
            ...
            try {
                ...
                Generate 4 audit sections: Execution Velocity, Resource Efficiency, Strategic Alignment, System Health.
                Use HTML tags (<b>, <br>, <ul>, <li>) for formatting.
                `;
    ...
    const DetailModal: React.FC<{ section: AuditSection; onClose: () => void; }> = ({ section, onClose }) => {
        ...
        return (
            <div className={`${isEmbedded ? 'absolute' : 'fixed'} inset-0 z-50 flex items-center justify-center bg-black/60 backdrop-blur-sm`} onClick={handleBackdropClick}>
                <div className="bg-[#111111]/90 backdrop-blur-2xl border border-white/10 rounded-lg shadow-2xl w-full max-w-2xl m-4 page-fade-in flex flex-col max-h-[90vh]">
                    ...
                    <div className="p-6 overflow-y-auto text-gray-300 leading-relaxed" dangerouslySetInnerHTML={{ __html: section.content }} />
                </div>
            </div>
        );
    };
    ...
    return (
        <div className={`${isEmbedded ? 'h-full flex flex-col p-4' : 'container mx-auto px-6 py-16 min-h-screen'}`}>
            {!isEmbedded && (
            ...
            <main className={`max-w-5xl mx-auto w-full ${isEmbedded ? 'flex-1 overflow-y-auto' : ''}`}>
                {isScanning ? (
                    ...
                    <div className="space-y-8">
                        ...
                        <div className="bg-[#111] border border-white/10 rounded-lg p-6 mt-8">
                            ...
                            <p className="text-sm text-gray-400 mb-6">
                                Sichern Sie den gesamten Systemstatus (Strategien, Aufgaben, Kontakte) lokal.
                            </p>
                            <div className="flex flex-col sm:flex-row gap-4">
                                <button onClick={exportSystemData} className="flex items-center justify-center gap-2 bg-green-600 hover:bg-green-500 text-white px-6 py-3 rounded-lg font-medium transition-colors shadow-lg shadow-green-900/20">
                                    <DownloadIcon /> System Backup
                                </button>
                                <label className="flex items-center justify-center gap-2 bg-[#222] hover:bg-[#333] text-gray-300 border border-[#444] px-6 py-3 rounded-lg font-medium transition-colors cursor-pointer">
                                    <UploadIcon />
