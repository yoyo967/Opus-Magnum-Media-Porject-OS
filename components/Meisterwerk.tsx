    ...
    return (
        <div className="bg-[#1C1C1C] rounded-lg p-4 border border-[#333333] flex flex-col md:flex-row gap-4 items-start">
            ...
            <div className="flex-1">
                ...
                <p className="text-xs text-gray-400 mb-3">Published on: {new Date(task.publishedAt!).toLocaleDateString()}</p>
                <div className="grid grid-cols-2 lg:grid-cols-4 gap-3 text-center">
                    <div><p className="text-xl font-bold text-white">{data.impressions?.toLocaleString()}</p><p className="text-xs text-gray-500">Impressions</p></div>
                    <div><p className="text-xl font-bold text-white">{data.engagementRate}%</p><p className="text-xs text-gray-500">Engagement</p></div>
                    <div><p className="text-xl font-bold text-white">{data.clicks?.toLocaleString()}</p><p className="text-xs text-gray-500">Clicks</p></div>
        ...
        <div id={`task-card-${task.id}`} draggable onDragStart={(e) => onDragStart(e, task.id)} onClick={() => onClick(task)} className={`bg-[#0A0A0A] p-3 rounded-md border  cursor-pointer hover:border-white/50 transition-colors space-y-2 highlight-new-task relative group ${task.isSystemGenerated ? 'border-purple-500/50' : 'border-[#333333]'} ${pulseAnimation}`}>
            <div className="absolute top-2 right-2 flex items-center gap-2">
                {task.isSystemGenerated && (
                    <div className="text-purple-400" title="Generated by AURORA">
                        <AuroraIcon />
                    </div>
                )}
                {task.chatHistory && task.chatHistory.length > 0 && (
                    <div className="text-gray-500 group-hover:text-blue-400 transition-colors" title="Conversator history available">
                ...
                <div className="flex items-center gap-2">
                    {recommendedToolConfig ? (
                        ...
                        task.imageUrl && !task.videoUrl && (
                            <button onClick={(e) => { e.stopPropagation(); onOpenInTool('animator', task); }} className="flex items-center gap-1 text-xs text-orange-400 hover:text-orange-300 transition-colors" title="Animate Image">
                                <MovieIcon /><span>Animate</span>
                            </button>
                        )
                    )}
                     {task.imageUrl && (
                        <button
                            onClick={(e) => { e.stopPropagation(); onOpenInTool('experimentator', task); }} className="flex items-center gap-1 text-xs text-teal-400 hover:text-teal-300 transition-colors" title="Create A/B Test">
                            <AbTestIcon /><span>Test</span>
                        </button>
                    )}
                </div>
                <div className="flex items-center gap-2">
                    {isReviewable && ( <button onClick={(e) => { e.stopPropagation(); onUpdateTask(task.id, { status: 'review' }); }} className="flex items-center gap-1 text-xs text-yellow-400 hover:text-yellow-300 transition-colors"><ReviewIcon /><span>Request Review</span></button> )}
                    {task.status === 'done' && task.isApproved && !task.publishedAt && ( <button onClick={(e) => { e.stopPropagation(); onPublish(task.id); }} className="flex items-center gap-1 text-xs text-cyan-400 hover:text-cyan-300 transition-colors"><PublishIcon /><span>Schedule</span></button> )}
                    {task.publishedAt && <span className="text-xs text-gray-500">Scheduled</span>}
                </div>
            </div>
        </div>
    );
    ...
    return (
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/60 backdrop-blur-sm" onClick={onClose}>
            <div onClick={e => e.stopPropagation()} className="bg-[#1C1C1C]/80 backdrop-blur-2xl border border-white/10 rounded-lg shadow-2xl w-full max-w-4xl h-[85vh] m-4 flex flex-col page-fade-in">
                <header className="p-4 border-b border-white/10 flex justify-between items-center"><h3 className="text-lg font-medium text-white">Task Details {isReviewMode && <span className="text-yellow-400 text-sm">(Review Mode)</span>}</h3><button onClick={onClose} className="text-gray-400 hover:text-white text-2xl">&times;</button></header>
                <div className="flex-1 p-6 grid md:grid-cols-2 gap-6 overflow-hidden">
                     <div className="flex flex-col gap-4 overflow-y-auto pr-2">
                        ...
                        <div className="flex gap-2 flex-wrap">
                            <button onClick={handleSendToResonator} className="text-xs bg-purple-600/20 text-purple-300 border border-purple-500/30 px-3 py-1.5 rounded-full hover:bg-purple-600/40 transition-colors flex items-center gap-1">
                                <MegaphoneIcon /> Convert to Resonator
                            </button>
                        </div>
                        ...
                        <div className="grid grid-cols-2 gap-4">
                            ...
                            <div>
                                <label className="text-xs font-medium text-gray-400">Actual Cost (€)</label>
                        ...
                        <div>
                            <h4 className="text-sm font-medium text-gray-400 mb-2">Checklist ({completedChecklistItems} / {totalChecklistItems})</h4>
                    ...
                    {isReviewMode ? (
                        <div className="flex flex-col bg-[#0A0A0A] border border-yellow-500/30 rounded-md overflow-hidden">
                             <h4 className="text-sm font-semibold text-white p-3 border-b border-[#333333] bg-black/20">Feedback & Approval</h4>
                             ...
                             <div className="p-3 border-t border-[#333333] space-y-2">
                                 <textarea value={feedbackInput} onChange={e => setFeedbackInput(e.target.value)} placeholder="Provide feedback..." rows={2} className="w-full bg-[#1C1C1C] text-white px-3 py-1.5 rounded-md border border-[#333333] text-sm"/>
                                 <div className="grid grid-cols-2 gap-2">
                                     <button onClick={() => handleAddFeedback('Reviewer')} className="bg-white/10 text-white text-xs py-1.5 rounded-md hover:bg-white/20">Send Comment</button>
                             ...
                             <div className="p-3 grid grid-cols-2 gap-3 bg-black/20">
                                 <button onClick={handleRequestChanges} className="bg-yellow-500/80 text-black font-bold py-2 rounded-md hover:bg-yellow-500 text-sm">Request Changes</button>
                                 <button onClick={handleApprove} className="bg-green-500/80 text-black font-bold py-2 rounded-md hover:bg-green-500 text-sm">Approve</button>
                             </div>
                        </div>
                    ) : (
...
const Meisterwerk: React.FC<MeisterwerkProps> = ({ navigateTo, isEmbedded }) => {
    ...
    const columnStyles: { [key in TaskStatus]: { title: string; headerColor: string; } } = { 
        todo: { title: 'To Do', headerColor: 'border-t-blue-500' }, 
        inprogress: { title: 'In Progress', headerColor: 'border-t-yellow-500' }, 
        review: { title: 'In Review', headerColor: 'border-t-orange-500' }, 
        done: { title: 'Done', headerColor: 'border-t-green-500' } 
    };
    ...
    return (
        <div className={`${isEmbedded ? 'p-4' : 'container mx-auto px-6 py-16'} h-full flex flex-col`}>
            ...
            <div className="border-b border-[#333333] mb-8 pb-4 flex items-center gap-2 flex-shrink-0">
                <ViewModeButton mode="strategy" title="Strategy" icon={<StrategyIcon />} />
                ...
                <div className="page-fade-in max-w-4xl mx-auto">
                    {campaignBrief ? (
                        <div className="space-y-6">
                            ...
                            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
                                {(campaignBrief.keyVisuals || []).map((visual, i) => (
                                    <ResultCard key={i} title={`Visual Idea #${i + 1}`}>
                                        <p>{visual}</p>
                                    </ResultCard>
                                ))}
                            </div>
                             <ResultCard title="Social Media Strategy">
                                <p><strong>Platforms:</strong> {(campaignBrief.socialMediaStrategy?.platforms || []).join(', ')}</p>
                             ...
                             <div className="grid md:grid-cols-2 gap-4">
                                <ResultCard title="Email Marketing">
                                    <p><strong>Subject Lines:</strong></p>
                                    <ul className="list-disc list-inside pl-2">
                                        {(campaignBrief.emailMarketing?.subjectLines || []).map((line, i) => <li key={i}>{line}</li>)}
                                    </ul>
                                    <p className="mt-2"><strong>Sequence Idea:</strong> {campaignBrief.emailMarketing?.sequenceIdea}</p>
                                </ResultCard>
                                <ResultCard title="Success Measurement (KPIs)">
                        ...
                        <div className="text-center py-20 bg-[#1C1C1C] rounded-lg border border-dashed border-[#333333]">
                            <h3 className="text-xl font-semibold text-white">No Campaign Brief Active</h3>
                            <p className="text-gray-400 mt-2">Define a strategy first to see it here.</p>
                        ...
                        {(Object.keys(columns) as TaskStatus[]).map((status) => (
                            <div key={status} onDrop={(e) => handleDrop(e, status)} onDragOver={(e) => { e.preventDefault(); setDragOverColumn(status); }} onDragLeave={() => setDragOverColumn(null)} className={`bg-[#1C1C1C] rounded-lg border transition-colors flex flex-col ${dragOverColumn === status ? 'border-white/50' : 'border-[#333333]'}`}>
                                ...
                                <div className="p-4 space-y-4 min-h-[200px] flex-1 overflow-y-auto">
                                    ...
                                    ) : ( <div className="flex items-center justify-center h-20 text-xs text-gray-500 text-center">No tasks in this status.</div> )}
                                </div>
                            </div>
                        ))}
                    </div>
                ...
                <div className="page-fade-in space-y-8">
                    {(() => {
                        ...
                        if (performanceTasks.length === 0) {
                             return (
                                <div className="text-center py-20 bg-[#1C1C1C] rounded-lg border border-dashed border-[#333333]">
                                    <h3 className="text-xl font-semibold text-white">No Performance Data</h3>
                                    <p className="text-gray-400 mt-2">Publish tasks from the board to see their performance here.</p>
                                </div>
                            );
                        }
                        ...
                        return (
                            <>
                                <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
                                    <Stat label="Total Impressions" value={totalImpressions.toLocaleString()} />
                                    <Stat label="Total Clicks" value={totalClicks.toLocaleString()} />
                                    <Stat label="Avg. Engagement" value={`${avgEngagement.toFixed(1)}%`} />
